...
export class MujerComponent implements OnInit{
  ...
  ngOnInit() {
  this.cargarProductosMujer();
  this.cargarTallasMujer();
  this.cargarTiposMujer();
  ...
  cargarProductosMujer(){
    this.productoService.getPorCategoria("mujer").subscribe(res=>{
      this.productos = res;
      this.filtrados = res;
    });
  }
